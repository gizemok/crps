...
def stream_and_filter(target_gb=10, chunk_gb=4.0, output_prefix="radar_massive_corpus", state_file="radar_stream_state.json"):
    ...
    # CSV Index Ayarlari (Insanlarin okumasi icin)
    ...
    if not csv_exists:
        csv_writer.writerow(["Matched_Keywords", "ArXiv_URL", "Timestamp"])
    ...
    for item in dataset:
        ...
        # Kelimeleri tarayip eslesenleri bul
        ...
        if matched_kws:
            ...
            # CSV dosyasina kaydet (Insan okumasi icin)
            url = meta.get("url", "Unknown_URL")
            timestamp = meta.get("timestamp", "Unknown_Time")
            csv_writer.writerow([", ".join(matched_kws), url, timestamp])
